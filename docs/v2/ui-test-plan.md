...
- **Smoke Test 記錄**：請於此處填入最新一次正式測試的 `reportId`、`runId`、`eventId`、Rich Menu 截圖 URL 及時間戳。  
  2025-11-14 Smoke Test（正式帳 / LIFF artifact `?ts=20251114T0355`）：
    - reportId=0f4915e3-d9ec-4a5f-91b7-f3215ff82fe1（Reports 模組同一筆）
    - guardian_report_refresh runId=80843760-…（prod, `test_mode=false`）／bfbd845c-…（測試模式）
    - guardian_plan_events eventId=27b8329a-…（Admin 模組顯示 `LINE 推播已排程`）
    - Rich Menu 截圖：../richmenu_starengine_v2.png
    - Quicklog：mem-quicklog id=20251113_012502（紀錄 S0/S7 CSS hotfix + LIFF deploy）
  後續若需新一輪 smoke test，請沿用上述格式並補充 LINE requestId / 截圖連結。
...
## 6. Mobile 版面更新（2025-11-14）
- **S0 表單**：`.lead-actions` 改為三等寬按鈕（「分析結果」/「預覽流程」/「啟動每日監控 🔍」）。手機寬度需呈現直向堆疊，無左傾。若仍出現不等寬，請確認 CSS `?ts=20251114T0355` 是否載入。
...
- **驗證步驟**：開啟 https://chelov134999.github.io/star-engine-liff-pages/index.html?ts=20251114T0355 ，以手機模擬器或 LINE LIFF 實機逐頁檢查 S0→S7；若任一頁仍看到舊樣式，請清除 LINE LIFF WebView cache（關閉視窗後重新開啟）。此 build 與 quicklog `20251113_012502` 對應，`.lead-actions` 與 `styles/s7-chatkit.css` padding 已同步。
